...
import { ProjectType, CountertopConfig, StaircaseConfig, ClientQuote, StoneMaterial } from './types';
...
export default function App() {
  ...
  // Handle standard resets for quick demos
  const handleReset = () => {
    if (projectType === 'pia') {
      setCountertop({
        width: 180,
  ...
  // --- DYNAMIC PHYSICS CALCULATIONS FOR TECHNICAL BLUEPRINTS ---
  ...
  const pesoEst = useMemo(() => {
    // Standard ornamental stone density is ≈ 2700 kg/m³
    ...
    <div className="min-h-screen bg-[#09090b] text-zinc-100 flex flex-col font-sans selection:bg-blue-600 selection:text-white select-none">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 p-6">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6 overflow-y-auto max-h-[calc(100vh-120px)] pr-1">
                  ...
                  <div className="bg-black/20 border border-white/5 p-4 rounded-xl space-y-3">
                    ...
                    {countertop.hasSink && (
                      <div className="space-y-3 pt-3 border-t border-white/5">
                        ...
                        {/* Lateral alignment offset slider */}
                        <div className="space-y-1">
                          <div className="flex justify-between text-[10px] text-zinc-500 font-bold uppercase tracking-wider">
          ...
          {/* D. BUDGET SUMMARY CARD ALIGNED WITH ASIDE TECHNICAL THEME */}
        ...
        {/* RIGHT COLUMN: 3D VIEWPORT, MATERIALS SELECTION & 2D BLUEPRINT VIEW (7Cols on Desktop) */}
        <div className="lg:col-span-7 flex flex-col gap-6">
          
          {/* 1. INTERACTIVE 3D STAGE FOR POLISHED SPECIMENS */}
          <div className="h-[430px] sm:h-[480px] relative rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
            ...
            {/* Simulated telemetries overlays above viewport */}
            ...
            {/* Scene control buttons right overlay */}
            ...
            {/* Bottom dynamic physical metrics box overlay onto the viewport */}
          ...
          {/* 2. DIVERSE CATALOGUE OF STONE MATERIALS SPECIMENS */}
          <div className="bg-[#121214] border border-white/10 p-5 rounded-2xl flex flex-col gap-4 shadow-2xl">
            ...
            {/* Scrolling material units listing */}
          ...
          {/* 3. PRECISION 2D CAD SCHEMATICS BLUEPRINT COMPONENT */}
          <Editor2D
            type={projectType}
            countertop={countertop}
            staircase={staircase}
      ...
      {/* --- QUICK BOTTOM SUMMARY NOTIFICATION DRAWER --- */}
      ...
      {/* --- FLOATING PRINTABLE COMPACT PROPOSAL DRAWERMODAL --- */}
      {showInvoice && (
        <BudgetInvoice
          type={projectType}
          countertop={countertop}
